...
if (isset($_GET['post_id']) && isset($_GET['type'])) {
    ...
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            // Jika belum ada reaksi, masukkan reaksi baru dengan flag is_guest
            $action = 'inserted';
            $stmt = $conn->prepare("INSERT INTO likes (user_id, post_id, type, is_guest) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("iisi", $user_id, $post_id, $type, $is_guest);
            $stmt->execute();
        }
        
        // === PROSES NOTIFIKASI ===
        // Hanya buat notifikasi jika user bukan guest
        if (!$is_guest) {
            if ($action === 'removed') {
